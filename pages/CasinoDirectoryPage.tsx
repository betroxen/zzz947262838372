...
const CasinoCard: React.FC<{ casino: Casino; onSelect: (c: Casino) => void }> = ({ casino, onSelect }) => {
    ...
    return (
        <div 
            ...
            className={`group relative bg-[#0c0c0e] border ${borderClass} ${shadowClass} hover:border-white/40 hover:shadow-2xl rounded-2xl p-0 flex flex-col transition-all duration-300 cursor-pointer overflow-hidden hover:-translate-y-2 h-full`}
        >
            ...
            <div className="p-5 relative z-20">
                <div className="flex justify-between items-start">
                     <div className="relative">
                        <img 
                            src={casino.logo} 
                            alt={casino.name} 
                            className={`w-16 h-16 rounded-xl object-cover bg-[#000] border-2 ${isCrown ? 'border-neon-surge' : 'border-[#222]'}`}
                            loading="lazy"
                        />
                        {isVerified && (
                            <div className="absolute -bottom-2 -right-2 bg-[#0c0c0e] rounded-full p-1 border border-[#333]" title="Verified Operator">
                                <Icons.Verified className="w-4 h-4 text-neon-surge" />
                            </div>
                        )}
                    </div>
                    
                    <div className="text-right">
                         <div className={`font-orbitron text-2xl font-bold ${casino.rating >= 4.5 ? 'text-neon-surge text-glow' : 'text-white'}`}>
                            {casino.rating.toFixed(1)}
                        </div>
                        <div className="flex gap-0.5 justify-end mt-1">
                            {[...Array(5)].map((_, i) => (
                                <Icons.Star key={i} className={`w-3 h-3 ${i < Math.floor(casino.rating) ? 'fill-neon-surge text-neon-surge' : 'text-[#333]'}`} />
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-4">
                    <h3 className="font-orbitron text-lg font-bold text-white uppercase tracking-wide group-hover:text-neon-surge transition-colors">{casino.name}</h3>
                     {casino.specialRanking && (
                        <span className={`text-[10px] font-jetbrains-mono uppercase tracking-wider font-bold ${isCrown ? 'text-neon-surge' : 'text-purple-400'}`}>
                            // {casino.specialRanking}
                        </span>
                    )}
                </div>
            ...
            {/* Mid Section: Tags */}
            <div className="px-5 mb-4 flex flex-wrap gap-2 relative z-20">
                 {casino.tags.slice(0, 3).map(tag => (
                    <span key={tag} className="text-[10px] font-jetbrains-mono uppercase px-2 py-1 rounded bg-[#1a1a1a] text-text-secondary border border-[#333]">
                        {tag}
                    </span>
                ))}
            </div>
            ...
            <div className="mt-auto bg-[#111] border-t border-[#222] p-4 grid grid-cols-2 gap-4 relative z-20">
                <div>
                    <p className="text-[10px] text-text-tertiary font-jetbrains-mono uppercase mb-0.5">Welcome Bonus</p>
                    <p className="text-xs font-bold text-white truncate" title={casino.bonus}>{casino.bonus}</p>
                </div>
                <div className="text-right">
                     <p className="text-[10px] text-text-tertiary font-jetbrains-mono uppercase mb-0.5">Withdrawal</p>
                    <p className={`text-xs font-bold ${casino.withdrawalSpeed.includes('Instant') ? 'text-neon-surge' : 'text-white'}`}>{casino.withdrawalSpeed}</p>
                </div>
            </div>
    ...
    return (
        <div className="animate-fadeIn">
            ...
            <div className="relative rounded-3xl border border-[#333] bg-[#0c0c0e] overflow-hidden mb-8 shadow-2xl">
                ...
                <div className="p-6 md:p-10 flex flex-col lg:flex-row gap-8 items-start lg:items-center relative z-10">
                    <div className="relative shrink-0">
                        <img 
                            src={casino.logo} 
                            alt={casino.name} 
                            className="w-32 h-32 rounded-2xl border-4 border-[#1a1a1a] shadow-2xl bg-black object-cover"
                        />
                         {casino.specialRanking === 'ETERNAL CROWN' && (
                            <div className="absolute -top-3 -left-3 bg-black text-neon-surge p-2 rounded-full border border-neon-surge shadow-neon-glow-md">
                    ...
                    <div className="flex-1 space-y-3">
                        ...
                        <p className="text-text-secondary font-rajdhani text-lg max-w-2xl leading-relaxed">
                            {casino.description}
                        </p>
                        
                        <div className="flex flex-wrap gap-2 pt-2">
                            {casino.tags.map(tag => (
                                <span key={tag} className="px-3 py-1 rounded bg-[#1a1a1a] border border-[#333] text-xs text-text-tertiary font-jetbrains-mono uppercase">
                                    {tag}
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="w-full lg:w-auto bg-[#14131c]/50 p-6 rounded-2xl border border-[#333] flex flex-col items-center justify-center min-w-[200px] backdrop-blur-sm">
                ...
                {/* Status Bar */}
                <div className="border-t border-[#333] bg-[#111] px-6 py-3 flex flex-wrap gap-6 text-xs font-jetbrains-mono text-text-tertiary">
                     <span className="flex items-center gap-2">
                        <Icons.Activity className="w-4 h-4 text-neon-surge" /> Status: <span className="text-white">ONLINE</span>
                     </span>
                     <span className="flex items-center gap-2">
                        <Icons.Shield className="w-4 h-4 text-blue-500" /> License: <span className="text-white">{casino.license}</span>
                     </span>
                     <span className="flex items-center gap-2">
                        <Icons.Clock className="w-4 h-4 text-yellow-500" /> Est: <span className="text-white">{casino.established}</span>
                     </span>
                </div>
            </div>
        ...
        <div className="animate-fadeIn max-w-[1600px] mx-auto">
            <header className="mb-12 text-center max-w-4xl mx-auto">
                <div className="inline-flex items-center gap-3 px-4 py-1.5 border border-neon-surge/30 rounded-full bg-neon-surge/5 mb-6 backdrop-blur-sm">
                    ...
                    <span className="text-xs font-orbitron text-neon-surge uppercase tracking-[0.2em] font-bold">Database Online v2.4</span>
                </div>
                ...
                <p className="text-text-secondary font-rajdhani text-lg md:text-xl leading-relaxed max-w-2xl mx-auto">
            ...
            {/* Filter Bar */}
            <div className="sticky top-20 z-40 mb-8 bg-[#050505]/80 backdrop-blur-md border-y border-[#222] py-4 -mx-4 px-4 md:mx-0 md:px-0 md:rounded-xl md:border md:p-4">
                <div className="flex flex-col md:flex-row gap-4 justify-between">
                    ...
                    <div className="relative w-full md:w-72">
                        ...
                        <Input 
                            ...
                            className="pl-10 bg-[#111] border-[#333] focus:border-neon-surge h-10 text-xs font-jetbrains-mono"
                        />
                    </div>
                    ...
                    <div className="flex gap-2 shrink-0">
                        <select 
                            ...
                            className="h-10 bg-[#111] border border-[#333] rounded-lg text-xs font-jetbrains-mono text-white px-3 focus:border-neon-surge outline-none cursor-pointer"
                        >
